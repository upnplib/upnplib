...
TEST(Ifaddr4TestSuite, show_real_loopback_interface) {
    ...
    EXPECT_EQ(rc, (ULONG)ERROR_SUCCESS);
    ...
    for (::PIP_ADAPTER_ADDRESSES adapts_item = adaptsObj.addr;
        ...
        if (adapts_item->IfIndex == 1) {
            ...
            EXPECT_EQ(adapts_item->PhysicalAddressLength, (ULONG)0);
...
TEST(Ifaddr4TestSuite, get_default_fake_loopback_interface) {
    ...
    EXPECT_EQ(adapts->IfIndex, (IF_INDEX)1);
    ...
    EXPECT_EQ(ip_addr->sin_addr.s_addr, (ULONG)16777343)
    ...
    EXPECT_THAT(adapts->PhysicalAddress,
                ElementsAre((BYTE)0, (BYTE)0, (BYTE)0, (BYTE)0, (BYTE)0,
                            (BYTE)0, (BYTE)0, (BYTE)0));
    EXPECT_EQ(adapts->PhysicalAddressLength, (ULONG)0);
    EXPECT_EQ(adapts->Flags, (ULONG)IP_ADAPTER_IPV4_ENABLED);
    EXPECT_EQ((LONG)adapts->Mtu, -1);
    EXPECT_EQ(adapts->IfType, (IFTYPE)IF_TYPE_SOFTWARE_LOOPBACK);
    EXPECT_EQ(adapts->OperStatus, ::IfOperStatusUp);
    // Only AF_INET (IPv4) available here
    EXPECT_EQ(adapts->Ipv6IfIndex, (IF_INDEX)0);
}
...
TEST(Ifaddr4TestSuite, set_regular_fake_network_interface) {
    ...
    EXPECT_EQ(adapts->IfType, (IFTYPE)IF_TYPE_ETHERNET_CSMACD);
    ...
    EXPECT_EQ(ip_addr->sin_family, AF_INET);
    EXPECT_EQ(ip_addr->sin_port, 0);
    EXPECT_EQ(ip_addr->sin_addr.s_addr, (ULONG)0);
...
TEST_P(Ifaddr4ParamTestSuite, set_fake_network_if_with_wrong_ip) {
    ...
    EXPECT_EQ(ip_addr->sin_family, AF_INET);
    EXPECT_EQ(ip_addr->sin_port, 0);
    EXPECT_EQ(ip_addr->sin_addr.s_addr, (ULONG)16777343); // "127.0.0.1"
    EXPECT_EQ(adapts->FirstUnicastAddress->OnLinkPrefixLength, 8);
    EXPECT_EQ((LONG)adapts->Mtu, -1);
    EXPECT_EQ(adapts->IfType, (IFTYPE)IF_TYPE_SOFTWARE_LOOPBACK);
}
...
TEST(NetIfs4ChainTestSuite, add_fake_interface_to_chain) {
    ...
    EXPECT_EQ(p_netif->IfIndex, (IF_INDEX)20);
    ...
    EXPECT_EQ(p_netif->IfIndex, (IF_INDEX)30);
    EXPECT_EQ(p_netif->Next, nullptr);
    ...
    EXPECT_EQ(p_netif->IfIndex, (IF_INDEX)1);
    EXPECT_EQ(p_netif->Next->IfIndex, (IF_INDEX)20);
    EXPECT_EQ(p_netif->Next->Next->IfIndex, (IF_INDEX)30);
