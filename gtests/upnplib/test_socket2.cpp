...
using rework::CSocket;
using rework::CSocket_basic;
...
TEST(SocketBasicTestSuite, instantiate_socket_successful) {
    ...
    EXPECT_THAT(
        ...
        ThrowsMessage<std::runtime_error>(StartsWith("UPnPlib ERROR 1032!")));
    EXPECT_THAT(
        [&sockObj]() { sockObj.get_port(); },
        ThrowsMessage<std::runtime_error>(StartsWith("UPnPlib ERROR 1026!")));
#endif
...
#else
    EXPECT_THAT(
        [sfd]() { CSocket_basic sockbObj(sfd); },
        ThrowsMessage<std::runtime_error>(StartsWith("UPnPlib ERROR 1014!")));
#endif
...
TEST(SocketBasicTestSuite, instantiate_socket_af_unix_sock_seqpacket) {
    ...
    EXPECT_EQ(sockObj.get_port(), 0);
    EXPECT_EQ(sockObj.get_family(), AF_UNIX);
    EXPECT_EQ(sockObj.get_type(), SOCK_SEQPACKET);
    EXPECT_EQ(sockObj.get_sockerr(), 0);
    EXPECT_FALSE(sockObj.is_reuse_addr());
#else
    EXPECT_THAT(
...
TEST(SocketBasicTestSuite, instantiate_socket_af_unix_sock_packet) {
    // The GCC compiler accepts with family AF_UNIX the types SOCK_STREAM,
    // SOCK_DGRAM, SOCK_RAW (changes silently to SOCK_DGRAM) and
    // SOCK_SEQPACKET.
    SOCKET sfd = ::socket(AF_UNIX, SOCK_RDM, 0);
...
TEST(SocketBasicTestSuite, try_instantiate_socket_with_invalid_fd) {
    EXPECT_THAT(
        []() { CSocket_basic sockbObj((SOCKET)~0 - 1); },
        ThrowsMessage<std::runtime_error>(StartsWith("UPnPlib ERROR 1014!")));
}
...
#if 0 // old tests from test_socket.cpp
//-------------------------------------
