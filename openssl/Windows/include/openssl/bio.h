...
#define sk_BIO_value(sk, idx)                                                  \
    ((BIO*)OPENSSL_sk_value(ossl_check_const_BIO_sk_type(sk), (idx)))
#define sk_BIO_new(cmp)                                                        \
    ((STACK_OF(BIO)*)OPENSSL_sk_new(ossl_check_BIO_compfunc_type(cmp)))
#define sk_BIO_new_null() ((STACK_OF(BIO)*)OPENSSL_sk_new_null())
#define sk_BIO_new_reserve(cmp, n)                                             \
    ((STACK_OF(BIO)*)OPENSSL_sk_new_reserve(ossl_check_BIO_compfunc_type(cmp), \
                                            (n)))
#define sk_BIO_reserve(sk, n)                                                  \
...
#define sk_BIO_delete(sk, i)                                                   \
    ((BIO*)OPENSSL_sk_delete(ossl_check_BIO_sk_type(sk), (i)))
#define sk_BIO_delete_ptr(sk, ptr)                                             \
    ((BIO*)OPENSSL_sk_delete_ptr(ossl_check_BIO_sk_type(sk),                   \
                                 ossl_check_BIO_type(ptr)))
#define sk_BIO_push(sk, ptr)                                                   \
...
#define sk_BIO_pop_free(sk, freefunc)                                          \
    OPENSSL_sk_pop_free(ossl_check_BIO_sk_type(sk),                            \
                        ossl_check_BIO_freefunc_type(freefunc))
#define sk_BIO_insert(sk, ptr, idx)                                            \
    OPENSSL_sk_insert(ossl_check_BIO_sk_type(sk), ossl_check_BIO_type(ptr),    \
                      (idx))
#define sk_BIO_set(sk, idx, ptr)                                               \
    ((BIO*)OPENSSL_sk_set(ossl_check_BIO_sk_type(sk), (idx),                   \
                          ossl_check_BIO_type(ptr)))
#define sk_BIO_find(sk, ptr)                                                   \
...
#define sk_BIO_find_all(sk, ptr, pnum)                                         \
    OPENSSL_sk_find_all(ossl_check_BIO_sk_type(sk), ossl_check_BIO_type(ptr),  \
                        pnum)
...
#define sk_BIO_dup(sk)                                                         \
    ((STACK_OF(BIO)*)OPENSSL_sk_dup(ossl_check_const_BIO_sk_type(sk)))
#define sk_BIO_deep_copy(sk, copyfunc, freefunc)                               \
    ((STACK_OF(BIO)*)OPENSSL_sk_deep_copy(                                     \
        ossl_check_const_BIO_sk_type(sk),                                      \
        ossl_check_BIO_copyfunc_type(copyfunc),                                \
        ossl_check_BIO_freefunc_type(freefunc)))
#define sk_BIO_set_cmp_func(sk, cmp)                                           \
    ((sk_BIO_compfunc)OPENSSL_sk_set_cmp_func(                                 \
        ossl_check_BIO_sk_type(sk), ossl_check_BIO_compfunc_type(cmp)))


/* Prefix and suffix callback in ASN1 BIO */
typedef int asn1_ps_func(BIO* b, unsigned char** pbuf, int* plen, void* parg);

typedef void (*BIO_dgram_sctp_notification_handler_fn)(BIO* b, void* context,
                                                       void* buf);
#ifndef OPENSSL_NO_SCTP
...
#define BIO_dgram_get_local_addr_enable(b, penable)                            \
    (int)BIO_ctrl((b), BIO_CTRL_DGRAM_GET_LOCAL_ADDR_ENABLE, 0,                \
                  (char*)(penable))
...
__owur int BIO_recvmmsg(BIO* b, BIO_MSG* msg, size_t stride, size_t num_msg,
                        uint64_t flags, size_t* msgs_processed);
int BIO_gets(BIO* bp, char* buf, int size);
int BIO_get_line(BIO* bio, char* buf, int size);
int BIO_write(BIO* b, const void* data, int dlen);
int BIO_write_ex(BIO* b, const void* data, size_t dlen, size_t* written);
__owur int BIO_sendmmsg(BIO* b, BIO_MSG* msg, size_t stride, size_t num_msg,
                        uint64_t flags, size_t* msgs_processed);
...
OSSL_DEPRECATEDIN_1_1_0 int BIO_get_port(const char* str,
...
int BIO_new_bio_pair(BIO** bio1, size_t writebuf1, BIO** bio2,
                     size_t writebuf2);
#ifndef OPENSSL_NO_DGRAM
int BIO_new_bio_dgram_pair(BIO** bio1, size_t writebuf1, BIO** bio2,
                           size_t writebuf2);
#endif
...
int BIO_meth_set_sendmmsg(BIO_METHOD* biom,
                          int (*f)(BIO*, BIO_MSG*, size_t, size_t, uint64_t,
                                   size_t*));
